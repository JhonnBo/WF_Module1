...
*/
        ...
        public Form1()
        {
            ...
            result = MessageBox.Show("Нажмите на одну из кнопок", "Окна сообщений",
                MessageBoxButtons.AbortRetryIgnore, MessageBoxIcon.Question);
            if (result == DialogResult.Abort)
            {
                MessageBox.Show("Нажата кнопка Прервать", "Окна сообщений",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else if (result == DialogResult.Retry)
            {
                MessageBox.Show("Нажата кнопка Повтор", "Окна сообщений",
                    MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else if (result == DialogResult.Ignore)
            {
                MessageBox.Show("Нажата кнопка Пропустить", "Окна сообщений",
